...
public class Storage extends Node {
    ...
    public void initAGV(int amount, int value) {
        for (int i = 0; i < amount; i++) {
            ...
            garageA.add(agv);
            agv.setLocalTranslation(location.x + 8 - (i * 5), location.y, 320);
            this.attachChild(agv);
        }
        for (int i = 0; i < amount; i++) {
            AGV agv = new AGV("SB" + Character.toChars(48 + value) + "V" + i, assetManager);
            garageB.add(agv);
            agv.setLocalTranslation(location.x + 8 - (i * 5), location.y, -260);
    ...
    public void initLine() {
        ...
        containerlines_geom.setLocalTranslation(this.location);
            ...
            for (int z = 0; z < 5; z++) {
                for (int y = 0; y <= 2; y++) {
                    if (this.containers.get(x).get(z).size() < 2) {
                        ...
                        c.setLocalTranslation(this.location.x - 6.5f + (2.8f * x), this.location.y + (y * 3.8f), this.location.z + 250 - (18.8f * z));
                        return;
                    }
                }
